...
Material::Material(const wchar_t* vertex_shader_path, const wchar_t* pixel_shader_path)
{
    ...
    m_vertex_shader = GraphicsEngine::get()->getRenderSystem()->createVertexShader(
        shader_byte_code, size_shader);
    GraphicsEngine::get()->getRenderSystem()->releaseCompiledShader();  // serbest býrak
    ...
    m_pixel_shader = GraphicsEngine::get()->getRenderSystem()->createPixelShader(
        shader_byte_code, size_shader);
...
void Material::setData(void* data, unsigned int size)
{
    if (!m_constant_buffer)
        ...
        m_constant_buffer->update(GraphicsEngine::get()->getRenderSystem()->getDevieContext(), data);
}
